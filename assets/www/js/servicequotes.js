...
$("div:jqmData(role='page')").on( "pageinit", function( event, ui ) {
    ...
    var pageId = this.id;
    debug && console.log( "ServiceQuotes.pageinit: Initializing " + pageId );
    // This MUST be called by every page specific pageinit event handler!
    UIFrame.init( pageId, function() {
        debug && console.log( "ServiceQuotes.pageinit: Page specific init goes here" );
