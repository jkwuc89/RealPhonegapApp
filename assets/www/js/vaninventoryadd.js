...
var VanInventoryAdd = function() {
    ...
    // Inventory array filtered by currently selected guided search
    var guidedSearchFilteredInventoryArray = null;

    /**
     * Perform the inventory load and display after the page loads
     */
    $(window).load( function() {
        Dialog.showPleaseWait( Localization.getText( "inventory" ), Localization.getText( "loadingInventoryFromDB" ), '400px' );
        MobileDb.selectData( MobileDb.SQL_SELECT_INVENTORY_PRODUCTS_STOCKAREAS_AVAILABLE.replace( "technicianBranchId", JSONData.getTechnicianBranchId() ), null, function( results ) {
            Dialog.closeDialog();
            ...
            debug && console.log( "VanInventoryAdd.window.load: Inventory product and stock area query returned " + results.length + " rows" );
            ...
            // Populate the page
            debug && console.log( "VanInventoryAdd.window.load: Calling populatePage()" );
            populatePage();
            ...
            debug && console.log( "VanInventoryAdd.window.load: Calling UIFrame.postPageSpecificInit()" );
    ...
    function populatePage( newInventoryList, repopulatingList ) {
        ...
        if( !repopulatingList ) {
            ...
            $('#btnDone').click( function() {
                ...
                var woLineForProduct = null,
                    currentWorkOrder = WorkOrder.getManageWorkOrder();
                ...
                for( var part in desiredParts ) {
                    if( desiredParts[part] != undefined && desiredParts[part].id != undefined ) {
                        ...
                        if( desiredParts[part].desiredQuantity <= inventory.quantity ) {
                            woLineForProduct = WorkOrder.createNewWorkOrderLine();
                            ...
                            WorkOrder.addLineToWorkOrder( currentWorkOrder, woLineForProduct);
                ...
                JSONData.saveInventoryQuantityChanges( inventoryChanges, function() {
                    UIFrame.navigateToPage( "manageworkorderparts.html", false, null );
                });
            });
        } else {
        ...
        if( workingInventory ) {
            for( var i = 0; i < workingInventory.length; i++ ) {
                ...
                if ( _.isFunction( currentInventory.description ) ) {
    ...
    function clearGuidedSearch( event ) {
        ...
        guidedSearchFilteredInventoryArray = null;
        var searchBox = $( "#inventorySearch" );
        var searchVal = searchBox.val();
        UIFrame.resetGuidedSearch( this );
        populatePage( null, true );
        fixAddButtons();
        // Reapply the search box criteria
        if ( searchVal ) {
    ...
    function guidedSearchClickHandler( event ) {
        ...
        if( this.id.indexOf("stockArea") != -1 ) {
            ...
            guidedSearchFilteredInventoryArray = _.filter( inventoryArray, function( currentInventory ) {
                return currentInventory.stockAreaId == stockAreaId;
            });
        }

        currentPage = 1;
        totalPages = parseInt(( guidedSearchFilteredInventoryArray.length + ( LIST_ITEM_NUMBER - 1 )) / LIST_ITEM_NUMBER );

        // Re-Process the inventory
        populatePage( guidedSearchFilteredInventoryArray, true );
        initializePaging();
        fixAddButtons();
