...
var ManageWorkOrderParts = function() {
    ...
    // Detect changes to the parts page
    ...
    function populatePage( pageId ) {
        ...
        var workOrderParts = _.filter( ManageWorkOrder.getWorkOrder().workOrderSegments[0].workOrderLines, function( workOrderLineInList ) {
            return workOrderLineInList.type == JSONData.WORK_ORDER_LINE_PART_TYPE;
        });
        ...
        if ( workOrderParts ) {
            ...
            for ( var partIndex = 0; partIndex < workOrderParts.length; partIndex++ ) {
                ...
                if ( currentWorkOrderPart.location ) {
                    ...
                    if ( currentWorkOrderPart.product.productCode == JSONData.OUTSIDE_PART_PURCHASE_PRODUCE_CODE ) {
                        currentWorkOrderPart.location = ko.observable( Localization.getText( "outsidePurchasePartLocation" ) );
                    } else {
                        currentWorkOrderPart.location =
                            ko.observable( JSONData.getInventoryLocationString( currentWorkOrderPart.inventoryId, partsInventory ) );
                    }
                } 
                currentWorkOrderPart.qtyOrdered = ko.observable( currentWorkOrderPart.qtyOrdered );
                currentWorkOrderPart.currentQty = currentWorkOrderPart.qtyOrdered();
                // A part is writable if its user ID matches the currently logged in technician
                // and the lineStatus is false.
                currentWorkOrderPart.writable = ko.observable( true );
                if ( currentWorkOrderPart.userId && currentWorkOrderPart.userId.length > 0 ) {
                    currentWorkOrderPart.writable( currentWorkOrderPart.userId == technicianUserId );
                }
                if ( currentWorkOrderPart.status != null && currentWorkOrderPart.status != undefined ) {
                    currentWorkOrderPart.writable( currentWorkOrderPart.status == 0 );
                }
                partsViewModel.parts.push( currentWorkOrderPart );
            }
        }
        
        // Set up the writable flag, the dirty flag and apply the knockout bindings
        // Parts page is made read only if the technician signed the work order.
        partsViewModel.writable( !ManageWorkOrder.isReadOnly() &&
    ...
    window.onbeforeunload = function() {
        if ( partsChanged ) {
            ...
            return Localization.getText( "leavingPartsPagePrompt" );
        }
    };
    ...
    function createWorkOrderLineForProduct( product ) {
        ...
        var woLineForProduct = JSONData.createNewWorkOrderLine();
     ...
     * @param action - action associated the location made inside this dialog (ADD_NEW_PART, CHANGE_PART_QTY, CHANGE_PART_LOCATION)
     * @param prompt - ID of prompt to display in the dialog 
     */
            ...
            switch ( action ) {
                case ADD_NEW_PART : 
                    ...
                    if ( PartMgmtState.vanQtyForPart > 0 ) {
                        ...
                        addPartToList( PartMgmtState.woLineForPart, true );
                    } 
                ...
                case CHANGE_PART_QTY :
                    ...
                    var newPart = jQuery.extend( true, {}, PartMgmtState.woLineForPart );
            ...
            if ( PartMgmtState.vanInventoryForPart ) {
                if ( PartMgmtState.vanInventoryForPart.quantity >= PartMgmtState.totalQtyForPart ) {
                    ...
                    if ( PartMgmtState.stockAreasForPart.length == 0 ) {
                        ...
                        workOrderLineForPart.inventoryId = PartMgmtState.vanInventoryForPart.webId;
                        addPartToList( workOrderLineForPart, true );
    ...
    function addPartToList( workOrderLineForPart ) {
        ...
        if ( workOrderLineForPart.product.productCode == JSONData.OUTSIDE_PART_PURCHASE_PRODUCE_CODE ) {
                ...
                function( products ) {
                    ...
                    if ( productsForNewQuickAddPart && productsForNewQuickAddPart.length > 0 ) {
                        ...
                        if ( productsForNewQuickAddPart.length == 1 ) {
                            ...
                            debug && console.log( "ManageWorkOrderParts.quickAddPartHandler: Loading mfg selection dialog" );
                            var dialogHtml = new EJS({url: 'templates/selectpartmfgdialog' }).render( {
                                partNumber : partsViewModel.partNumber(),
                                mfgs : mfgsForDlg
                            });
                            debug && console.log( "ManageWorkOrderParts.quickAddPartHandler: Showing mfg selection dialog" );
                            $(document).simpledialog2({
                                mode : 'blank',
                                blankContent : dialogHtml,
                                width : '600px'
                            });
                            debug && console.log( "ManageWorkOrderParts.quickAddPartHandler: Mfg selection dialog displayed" );
                        }
                    } else {
            ...
            if ( currentInventory.quantity < qtyChange ) {
                ...
                if ( inventoryInfoForChangedPart && inventoryInfoForChangedPart.length > 0 ) {
                    ...
                    Dialog.showConfirmYesNo( Localization.getText( "partsRequest" ), promptText,
                        function() {
                            // Add 2nd line for PARTS REQUEST quantity
                            var newPart = jQuery.extend( true, {}, partToChange );
    ...
    function saveChanges() {
        ...
        var newWorkOrderLines = _.reject( workOrder.workOrderSegments[0].workOrderLines, function( lineInList ) {
            return lineInList.type == JSONData.WORK_ORDER_LINE_PART_TYPE;
        });
     ...
     */
    function startPSRT() {        
    	Util.startPSRT( ManageWorkOrder.getEquipment().serialNumber, psrtModelCrossReference[ ManageWorkOrder.getEquipment().product.productCode ], 
    ...
    function openInventory() {
    	...
    	window.localStorage.setItem( "inventoryContext", "popup" );
    	UIFrame.navigateToPage( "vaninventoryadd.html" );	
    }
    ...
    function addOutsidePartPurchasesToPartList() {
        ...
        _.each( partsViewModel.outsidePurchasePartList(), function( partInList ) {
            ...
            if ( partInList.quantity() >= 1 &&
                ...
                workOrderLine = JSONData.createNewWorkOrderLine();
                workOrderLine.cost                = partInList.priceEach();
                workOrderLine.description         = partInList.description();
                workOrderLine.qtyOrdered          = partInList.quantity();
                workOrderLine.type                = JSONData.WORK_ORDER_LINE_PART_TYPE;
                workOrderLine.product.productCode = JSONData.OUTSIDE_PART_PURCHASE_PRODUCE_CODE;
    ...
    function addPSRTPartsToPartList() {
        var psrtStarted = ( window.localStorage.getItem( LS_PSRT_STARTED ) || Util.isRunningOnChromeDesktop() );
        if ( psrtStarted ) {
            // Remove the PSRT started flag.  This prevents other resume events
            // from mistakenly attempting to add PSRT parts
            window.localStorage.removeItem( LS_PSRT_STARTED );
            var cartFilename = PSRT_CART_DIR + "/" + ManageWorkOrder.getWorkOrder().documentNumber + ".xml";
            debug && console.log( "ManageWorkOrderParts.addPSRTPartsToPartList: Attempting to add parts from PSRT cart file: " +
                                  cartFilename );
            try {
                var baseCartFileUrl = "";
                if ( !Util.isRunningOnChromeDesktop() ) {
                    baseCartFileUrl = '/mnt/sdcard/';
                }
                $.ajax( {
                    url : baseCartFileUrl + cartFilename,
                    dataType : 'xml',
                    ...
                    success : function( cartXml, textStatus, jqXHR ) {
                        ...
                        var jsonPartRequestFile = "/mnt/sdcard/" + PSRT_REQUEST_DIR + "/" + 
                        ...
                        LocalFS.deleteFile( jsonPartRequestFile );
                        LocalFS.deleteFile( cartFilename );
                        ...
                        if ( items && items.length > 0 ) {
                            ...
                            _.each( items, function( itemInList ) {
                                var workOrderLine = JSONData.createNewWorkOrderLine();
