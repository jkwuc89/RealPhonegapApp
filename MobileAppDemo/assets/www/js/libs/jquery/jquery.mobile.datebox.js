...
(function($, undefined ) {
  ...
  $.widget( "mobile.datebox", $.mobile.widget, {
                        ...
                        if ( $.isArray(o.defaultPickerValue) && o.defaultPickerValue.length === 3 ) {
                            ...
                            if ( o.mode === 'timebox' || o.mode === 'timeflipbox' ) {
                                ...
                                if ( exp_temp.length === 3 ) {
                                    hours = parseInt(exp_temp[0],10);
                                    minutes = parseInt(exp_temp[1],10);
                                    seconds = parseInt(exp_temp[2],10);
                                    date = new Date(found_date[0], found_date[1], found_date[2], hours, minutes, seconds ,0);
                                    if ( isNaN(date.getDate()) ) {
                                        date = new Date();
                                    } else {
                                        found_date[3] = hours;
                                        found_date[4] = minutes;
                                        found_date[5] = seconds;
                                    }
                                }
                            }
                            else {
    ...
    _offset: function(mode, amount, update) {
        ...
        if ( ok === true ) {
            // Preserve the current date in case the new date is not valid
            self.oldDate = new Date( self.theDate );
            self.theDate.adjust(mode,amount);
        }
    ...
    _checkConstraints: function() {
        ...
        if ( o.beforeToday !== false ) {
            testDate = new Date();
            if ( self.theDate > testDate ) {
                // If new date is not valid, restore 
                // the old date if available, otherwise use the current date/time stamp
                if ( self.oldDate ) {
                    self.theDate = new Date( self.oldDate );
                } else {
                    self.theDate = new Date( testDate );
                }
            }
        }
        if ( o.maxDays !== false ) {
