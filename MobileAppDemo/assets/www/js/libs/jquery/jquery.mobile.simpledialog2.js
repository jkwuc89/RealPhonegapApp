...
(function($, undefined ) {
  $.widget( "mobile.simpledialog2", $.mobile.widget, {
	...
	open: function() {
		...
		// Adding rel=cancel option that allows dialog to be closed without calling the specified callback function
		self.sdIntContent.delegate('[rel=cancel]', o.clickEvent, function (e) { e.preventDefault(); self.close( true ); });
	...
	close: function( skipCallback ) {
		var self = this, retty;
		if ( !skipCallback ) {
		    skipCallback = false;
		}
		
		if ( !skipCallback && $.isFunction(self.options.callbackClose) ) {
