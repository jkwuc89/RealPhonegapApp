/**
 * timecard.js
 * TimeCard support script for JMeter tests
 */

// Constants
var TECHNICIAN_NOTES = "JMETER TimeCard test";
var signatureImage =
...
var VALID_LABOR_CODE_IDS = [ 3, 6, 14, 17, 19, 21, 24, 25 ];
function getRandomLaborCodeId() {
	return VALID_LABOR_CODE_IDS[ Math.floor(( Math.random() * VALID_LABOR_CODE_IDS.length )) ];
}

var VALID_ORDER_SEGMENT_IDS = [ 4224489, 4224560, 4224561, 4224562, 4224563, 4224564, 4224565, 4224566, 4224567, 4224568 ];
function getRandomSegmentId() {
	return VALID_ORDER_SEGMENT_IDS[ Math.floor(( Math.random() * VALID_ORDER_SEGMENT_IDS.length )) ];
}
...
function getSignature() {
	// Set the signatures on the time card
    var signature = {
        webId : null,
        dateCaptured : getISOCurrentTime(),
...
/**
 * Create a random number of timeCardLines
 * @return timeCardLines array
 */
function generateTimeCardLines( timeCardId ) {
	var numLines = Math.floor(( Math.random() * 8 )) + 1;
	var timeCardLines = [];
	var newTimeCardLine = {};
	var laborCodeId = null;
	var workOrderSegmentId = null;
	
	var clockingInterval = 1000 * 60 * 60;
	var startTime = new Date().getTime() - ( numLines * clockingInterval );
    var endTime = startTime + clockingInterval;
	
	for( var i = 0; i < numLines; i++ ) {
		laborCodeId = getRandomLaborCodeId();
		
		// If a non-productive type, workOrderSegmentId is null
		if( laborCodeId == 3 || laborCodeId == 6 ) {
			workOrderSegmentId = getRandomSegmentId();
		} else {
			workOrderSegmentId = null;
		}
		
		// Initialize a newTimeCardLine
		newTimeCardLine = {
			webId: getUniqueId(),
			internalId: "",
			timeStart: new Date( startTime ).toISOString(),
			timeEnd: new Date( endTime ).toISOString(),
			workOrderSegmentId: workOrderSegmentId,
		...
		timeCardLines.push( newTimeCardLine );
	}
...
/**
 * Create a new time card store the time card
 * inside the JMeter var newWorkOrder
 * @param args - JMeter args
 */
function createNewTimeCard( args ) {
    log.info( "CreateNewTimeCard: Creating new TimeCard" );
	
	var newTimeCard = {};
		newTimeCard.timeCard 						= {};
		newTimeCard.timeCard.webId 					= getUniqueId();
		newTimeCard.timeCard.internalId 			= "";
		newTimeCard.timeCard.clientReference	 	= newTimeCard.timeCard.webId;
		newTimeCard.timeCard.technicianNotes		= TECHNICIAN_NOTES;
		newTimeCard.timeCard.technicianSignature	= getSignature();
		newTimeCard.timeCard.timeCardLines		 	= generateTimeCardLines( newTimeCard.timeCard.webId );

	vars.put( "newTimeCard", JSON.stringify( newTimeCard ));
}
 ...
 */
var VALID_ACTIONS = {
    "createNewTimeCard" : createNewTimeCard
};
