...
function setTestResult( testPassed ) {
    ...
    if ( !testPassed ) {
        if ( workOrderNumber ) {
            testResultDescription = "Work Order " + workOrderNumber + " test result: " + testResultDescription;
        }
...
/**
 * Get the work order post response data from the previous
 * JMeter Sampler
 * @returns Work order response data parsed as a JSON object
 */
function getResponseData() {
    var updatedWorkOrder = null;
    try {
        updatedWorkOrder = JSON.parse( prev.getResponseDataAsString() );
    } catch ( exc ) {
        log.error( "getResponseData: Work order post returned invalid JSON data" );
    }
    return updatedWorkOrder;
}

/**
 * Validate the clockings returned in the updated work order
 * @param postedClockings
 * @param updatedClockings
 */
function validateClockings( postedClockings, updatedClockings ) {
    var valid = true;

    // Updated work order should have same number of clockings inside posted work order and
    // all clockings should have non-null internal IDs
    if ( updatedClockings && updatedClockings.length == postedClockings.length ) {
        for ( var i = 0; i < updatedClockings.length && valid; i++ ) {
            if ( updatedClockings[i].internalId == null ) {
               valid = false;
               testResultDescription = "validateClockings: Updated work order contains technicianClocking with internalId = null";
            }
        }
    } else {
        valid = false;
        testResultDescription = "validateClockings: Updated work order is missing technicianClockings";
    }
    return valid;
}

/**
 * Validate the parts returned in the updated work order
 * @param postedWOLines
 * @param updatedWOLines
 */
function validateParts( postedWOLines, updatedWOLines ) {
    var valid = true;
    if ( updatedWOLines.length && postedWOLines.length ) {
        var numPostedParts = 0;
        var postedPartNumbers = [];
        var numUpdatedParts = 0;
        var updatedPartNumbers = [];

        // Get the parts from the posted WO and the updated WO
        for ( var i = 0; i < postedWOLines.length && valid; i++ ) {
            if ( postedWOLines.type === WORK_ORDER_LINE_PART_TYPE ) {
                numPostedParts++;
                postedPartNumbers.push( postedWOLines[i].product.productCode );
            }
        }

        for ( var i = 0; i < updatedWOLines.length && valid; i++ ) {
            if ( updatedWOLines.type === WORK_ORDER_LINE_PART_TYPE ) {
                numUpdatedParts++;
                if ( updatedWOLines[i].product.productCode ) {
                    updatedPartNumbers.push( updatedWOLines[i].product.productCode );
                } else {
                    valid = false;
                    testResultDescription = "validateParts: Updated work order contains a part with missing product info";
                }
            }
        }

        // Compare the parts
        if ( numPostedParts === numUpdatedParts ) {
            postedPartNumbers.sort();
            updatedPartNumbers.sort();
            for ( var i = 0; i < postedPartNumbers.length && valid; i++ ) {
                if ( postedPartNumbers[i] !== updatedPartNumbers[i] ) {
                    valid = false;
                    testResultDescription = "validateParts: Part numbers in updated work order do not match part numbers in posted work order";
                }
            }
        } else {
            valid = false;
            testResultDescription = "validateParts: Number of parts in updated work order does not match number of parts in posted work order";
        }
    }
    return valid;
}
 ...
 */
function validateNewWorkOrderResponse() {
    var testSuccessful = true;
    // JMeter var newWorkOrder contains the new work order JSON that was posted
    var postedWorkOrder = JSON.parse( vars.get( "newWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        // Did Quipware update the work order?
        if ( updatedWorkOrder.workOrder.internalId == null ) {
            testSuccessful = false;
            testResultDescription = "validateNewWorkOrderResponse: Quipware did not update work order internalId";
        }
        if ( testSuccessful && updatedWorkOrder.workOrder.documentNumber.indexOf( "JMeter" ) !== -1 ) {
            testSuccessful = false;
            testResultDescription = "validateNewWorkOrderResponse: Quipware did not update work order documentNumber";
        }

        // Validate the work order header properties
        if ( testSuccessful ) {
            if ( !updatedWorkOrder.workOrder.contactName || updatedWorkOrder.workOrder.contactName !== "JMeter New WO Contact Name" ) {
                testSuccessful = false;
                testResultDescription = "validateNewWorkOrderResponse: Work order contact name is missing or invalid";
            } else if ( !updatedWorkOrder.workOrder.contactNumber || updatedWorkOrder.workOrder.contactNumber !== "614-555-1212 Ext: 2112" ) {
                testSuccessful = false;
                testResultDescription = "validateNewWorkOrderResponse: Work order contact number is missing or invalid";
            }
        }

        // Validate the work order segment properties
        if ( testSuccessful ) {
            var workOrderSegment = updatedWorkOrder.workOrder.workOrderSegments[0];
            if ( !workOrderSegment.segmentAction || workOrderSegment.segmentAction !== "000008" ) {
                testSuccessful = false;
                testResultDescription = "validateNewWorkOrderResponse: Work order segment action is missing or invalid";
            }
        }

        // Validate the technicianClockings
        if ( testSuccessful ) {
            testSuccessful = validateClockings( postedWorkOrder.technicianClocking, updatedWorkOrder.technicianClocking );
        }

        // Validate part lines
        if ( testSuccessful ) {
            testSuccessful = validateParts( postedWorkOrder.workOrder.workOrderSegments[0].workOrderLines,
                                            updatedWorkOrder.workOrder.workOrderSegments[0].workOrderLines );
        }

        if ( testSuccessful ) {
            testResultDescription = "validateNewWorkOrderResponse: Updated work order is valid";
            // Save the new work order response as a JMeter test var
            delete updatedWorkOrder.success;
            vars.put( "newWorkOrderResponse", JSON.stringify( updatedWorkOrder ) );
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateNewWorkOrderResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
 ...
 * Validate the response to posting a new work order
 */
function validateDuplicateNewWorkOrderResponse() {
    var testSuccessful = true;
    // Get work order data from JMeter
    var originalPostResponse = JSON.parse( vars.get( "newWorkOrderResponse" ) );
    var postedWorkOrder = JSON.parse( vars.get( "duplicateNewWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        // Did Quipware update the work order?
        if ( updatedWorkOrder.workOrder.internalId == null ) {
            testSuccessful = false;
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Quipware did not update work order internalId";
        }
        if ( testSuccessful && updatedWorkOrder.workOrder.documentNumber.indexOf( "JMeter" ) !== -1 ) {
            testSuccessful = false;
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Quipware did not update work order documentNumber";
        }

        // Did the middle tier return a different work order webId for the repost?
        if ( testSuccessful && updatedWorkOrder.workOrder.webId != originalPostResponse.workOrder.webId ) {
            testSuccessful = false;
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Original post webId " +
                                    originalPostResponse.workOrder.webId + " does not match repost webId " +
                                    updatedWorkOrder.workOrder.webId;
        }
        // Did the middle tier return a different work order number for the repost?
        if ( testSuccessful && updatedWorkOrder.workOrder.documentNumber != originalPostResponse.workOrder.documentNumber ) {
            testSuccessful = false;
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Original post documentNumber " +
                                    originalPostResponse.workOrder.documentNumber + " does not match repost documentNumber " +
                                    updatedWorkOrder.workOrder.documentNumber;
        }
        // Did the middle tier return a different internalId for the repost?
        if ( testSuccessful && updatedWorkOrder.workOrder.internalId != originalPostResponse.workOrder.internalId ) {
            testSuccessful = false;
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Original post internalId " +
                                    originalPostResponse.workOrder.internalId + " does not match repost internalId " +
                                    updatedWorkOrder.workOrder.internalId;
        }
        ...
        if ( testSuccessful ) {
            testResultDescription = "validateDuplicateNewWorkOrderResponse: Updated work order is valid";
        }
    } else {
        testResultDescription = "validateDuplicateNewWorkOrderResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
...
/**
 * Validate the response to accepting a work order
 */
function validateAcceptResponse() {
    var testSuccessful = true;
    var postedWorkOrder = JSON.parse( vars.get( "updatedWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        if ( updatedWorkOrder.workOrder.workOrderSegments[0].webStatus != ACCEPTED ) {
            testSuccessful = false;
            testResultDescription = "validateAcceptResponse: Updated work order status is not accepted (100)";
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateAcceptResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
...
/**
 * Validate the response to opening a work order
 */
function validateOpenResponse() {
    ...
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        if ( updatedWorkOrder.workOrder.workOrderSegments[0].webStatus != IN_PROGRESS ) {
            testSuccessful = false;
            testResultDescription = "validateOpenResponse: Updated work order status is not in progress (500)";
        }
        if ( testSuccessful && updatedWorkOrder.workOrder.workOrderSegments[0].dateStarted == null ) {
            testSuccessful = false;
            testResultDescription = "validateOpenResponse: Updated work order dateStarted is null";
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateOpenResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
...
/**
 * Validate the response to reassigning a work order
 */
function validateReassignmentResponse() {
    var testSuccessful = true;
    var postedWorkOrder = JSON.parse( vars.get( "updatedWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        if ( updatedWorkOrder.workOrder.workOrderSegments[0].webStatus != REJECTED ) {
            testSuccessful = false;
            testResultDescription = "validateReassignmentResponse: Updated work order status is not rejected (200)";
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateReassignmentResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
...
/**
 * Validate a signature inside a work order
 * @param signatureType - "technician" or "customer"
 * @param signature - signature object
 * @returns true or false
 */
function validateWorkOrderSignature( signatureType, signature ) {
    var valid = true;
    if ( signature ) {
        log.info( "validateWorkOrderSignature: Validating " + signatureType + " signature" );
        if ( !signature.webId ) {
            valid = false;
            testResultDescription = "validateWorkOrderSignature: " + signatureType + " signature webId is null or missing";
        }
        if ( valid && !signature.dateCaptured ) {
            valid = false;
            testResultDescription = "validateWorkOrderSignature: " + signatureType + " signature dateCaptured is null or missing";
        }
        if ( valid && signature.format != signatureFormat ) {
            valid = false;
            testResultDescription = "validateWorkOrderSignature: " + signatureType + " signature format is not set to " + signatureFormat;
        }
        if ( valid && signature.value != signatureImage ) {
            valid = false;
            testResultDescription = "validateWorkOrderSignature: " + signatureType + " signature image does not match what was posted";
        }
    } else {
        valid = false;
        testResultDescription = "validateWorkOrderSignature: " + signatureType + " signature is missing";
    }
    return valid;
}

/**
 * Validate the response to signing and closing a work order
 */
function validateSignAndCloseResponse() {
    var testSuccessful = true;
    var postedWorkOrder = JSON.parse( vars.get( "updatedWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    ...
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        log.info( "validateSignAndCloseResponse: Validating status and signatures of closed work order " + workOrderNumber );
        // Is work order closed?
        if ( updatedWorkOrder.workOrder.workOrderSegments[0].webStatus != CLOSED ) {
            testSuccessful = false;
            testResultDescription = "validateSignAndCloseResponse: Updated work order status is not closed (700)";
        }
        // Validate the technician signature
        if ( testSuccessful ) {
            testSuccessful = validateWorkOrderSignature( "technician",
                                                         updatedWorkOrder.workOrder.workOrderSegments[0].technicianSignature );
        }
        // Validate the customer signature
        if ( testSuccessful ) {
            testSuccessful = validateWorkOrderSignature( "customer",
                                                         updatedWorkOrder.workOrder.workOrderSegments[0].customerSignature );
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateSignAndCloseResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
...
/**
 * Validate the response to updating an existing work order
 */
function validateUpdatedWorkOrderResponse() {
    var testSuccessful = true;
    // JMeter var updatedWorkOrder contains the updated work order JSON that was posted
    var postedWorkOrder = JSON.parse( vars.get( "updatedWorkOrder" ) );
    var updatedWorkOrder = getResponseData();
    if ( updatedWorkOrder && updatedWorkOrder.success ) {
        workOrderNumber = updatedWorkOrder.workOrder.documentNumber;
        ...
        if ( testSuccessful ) {
            testResultDescription = "validateUpdatedWorkOrderResponse: Updated work order is valid";
        }
    } else {
        workOrderNumber = postedWorkOrder.workOrder.documentNumber;
        testResultDescription = "validateUpdatedWorkOrderResponse: Updated work order data indicates that a post failure occurred";
        testSuccessful = false;
    }
    setTestResult( testSuccessful );
}
 ...
 */
var VALID_ACTIONS = {
    "validateAcceptResponse" : validateAcceptResponse,
    "validateDuplicateNewWorkOrderResponse" : validateDuplicateNewWorkOrderResponse,
    "validateNewWorkOrderResponse" : validateNewWorkOrderResponse,
    "validateOpenResponse" : validateOpenResponse,
    "validateReassignmentResponse" : validateReassignmentResponse,
    "validateSignAndCloseResponse" : validateSignAndCloseResponse,
    "validateUpdatedWorkOrderResponse" : validateUpdatedWorkOrderResponse
};
