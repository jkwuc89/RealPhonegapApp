...
var Customers = function() {
    ...
    function populateListViews( customersList, repopulatingList ) {
       ...
       for ( var customer in customers ) {
           ...
           pmsDueCount = JSONData.getPMCountForCustomer( pmSchedule, currentCustomer.webId, true );
        ...
        $('.customer-list-item').on( "click", openCustomerEquipmentOnTap );
        ...
        if ( this.id.indexOf( "openPeriodic" ) != -1 ) {
            ...
            filteredCustomers = _.filter( allCustomers, function( currentCustomer ) {
                pmsDueCount = _.filter( pmSchedule, function( pmScheduleInList ) {
                    return ( JSONData.isPMBeforeCutoffDate( pmScheduleInList ) ) && pmScheduleInList.customerId == currentCustomer.webId;
                }).length;
            ...
            filteredCustomers = _.filter( allCustomers,  function( currentCustomer ) {
                return ( WorkOrder.getWorkOrderCountForCustomer( workOrders, currentCustomer.webId ) > 0 );
            });
        }
