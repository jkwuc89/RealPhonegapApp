...
var VanInventory = function() {
    ...
    $(window).load( function() {
        ...
        MobileDb.selectData( MobileDb.SQL_SELECT_INVENTORY_PRODUCTS_STOCKAREAS_AVAILABLE.replace( "technicianBranchId", JSONData.getTechnicianBranchId() ), null, function( results ) {
            Dialog.closeDialog();
            debug && console.log( "VanInventory.window.load: Inventory product and stock area query returned " + results.length + " rows" );
            ...
            // Populate the page
            debug && console.log( "VanInventory.window.load: Calling populatePage()" );
            populateListViews();
            ...
            debug && console.log( "VanInventory.window.load: Calling UIFrame.postPageSpecificInit()" );
    ...
    function guidedSearchClickHandler( event ) {
        ...
        currentPage = 0;
        if( guidedSearchFilteredInventoryArray == null ) {
            guidedSearchFilteredInventoryArray = [];
        } else {
            totalPages = parseInt( ( guidedSearchFilteredInventoryArray.length + (LIST_ITEM_NUMBER - 1) ) / LIST_ITEM_NUMBER );
            initializePaging();
        }

        populateListViews( guidedSearchFilteredInventoryArray, true );
