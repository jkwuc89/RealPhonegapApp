function switchToAndroid() {
    ...
    cat $1 | sed 's/cordova-2\.1\.0\.ios\.min\.js/cordova-2\.1\.0\.android\.min\.js/' > temp.html
    mv temp.html $1
}

function switchToiOS() {
    echo "Switching $1 to iOS version of Phonegap"
    cat $1 | sed 's/cordova-2\.1\.0\.android\.min\.js/cordova-2\.1\.0\.ios\.min\.js/' > temp.html
    mv temp.html $1
}
